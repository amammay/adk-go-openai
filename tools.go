...
func convertTools(cfg *genai.GenerateContentConfig) ([]responses.ToolUnionParam, error) {
	...
	for i, tool := range cfg.Tools {
		if err := ensureSupportedTool(i, tool); err != nil {
			return nil, err
		}
		...
		for j, server := range tool.MCPServers {
			mcpTool, err := convertMCPServer(i, j, server)
			if err != nil {
				return nil, err
			}
...
func ensureSupportedTool(idx int, tool *genai.Tool) error {
	if tool == nil {
	...
	if tool.Retrieval != nil || tool.GoogleSearch != nil || tool.GoogleSearchRetrieval != nil ||
		...
		return fmt.Errorf("openai: only function and mcp tools are supported (tool %d)", idx)
	}
	if len(tool.FunctionDeclarations) == 0 && len(tool.MCPServers) == 0 {
		return fmt.Errorf("openai: tool %d does not declare any function or mcp server", idx)
	}
	return nil
}

func convertMCPServer(toolIdx, serverIdx int, server *genai.MCPServer) (*responses.ToolMcpParam, error) {
	if server == nil {
		return nil, fmt.Errorf("openai: tool %d mcp server %d is nil", toolIdx, serverIdx)
	}
	if strings.TrimSpace(server.Name) == "" {
		return nil, fmt.Errorf("openai: tool %d mcp server %d missing name", toolIdx, serverIdx)
	}
	if server.StreamableHTTPTransport == nil {
		return nil, fmt.Errorf("openai: tool %d mcp server %d missing streamable http transport", toolIdx, serverIdx)
	}
	if strings.TrimSpace(server.StreamableHTTPTransport.URL) == "" {
		return nil, fmt.Errorf("openai: tool %d mcp server %d missing transport url", toolIdx, serverIdx)
	}

	tool := &responses.ToolMcpParam{
		ServerLabel: server.Name,
		ServerURL:   param.NewOpt(server.StreamableHTTPTransport.URL),
	}
	if len(server.StreamableHTTPTransport.Headers) > 0 {
		tool.Headers = make(map[string]string, len(server.StreamableHTTPTransport.Headers))
		for k, v := range server.StreamableHTTPTransport.Headers {
			tool.Headers[k] = v
		}
	}
	return tool, nil
